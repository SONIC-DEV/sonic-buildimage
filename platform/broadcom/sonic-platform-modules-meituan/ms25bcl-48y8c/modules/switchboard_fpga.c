/*
 * switchboard.c - Driver for b3010 Switch FPGA/CPLD.
 *
...
#define CLASS_NAME "b3010_fpga"
#define DRIVER_NAME "b3010.switchboard"
#define FPGA_PCI_NAME "b3010_fpga_pci"
...
/*
 * struct b3010_fpga_data - Private data for b3010 switchboard driver.
...
struct b3010_fpga_data *fpga_data;
...
static struct device * b3010_sff_init(int portid) {
 ...
 *
 * b3010 have 2 switch CPLDs. Each CPLD manages 28 ports.
 *
 *  +------------------+------------------+
                           ...
                           int size, union i2c_smbus_data *data)
{
    ...
    if(retry <= 0 && error == -EBUSY ){
        ...
        /**
         * b3010: Device specific I2C reset topology
         */
...
static const struct i2c_algorithm b3010_i2c_algorithm = {
                                            ...
                                            int bus_number_offset)
{
    ...
    new_adapter->algo  = &b3010_i2c_algorithm;
...
static struct i2c_adapter *b3010_i2c_init(struct platform_device *pdev,
                                                int portid,
                                                int bus_number_offset) {
...
// I/O resource need.
static struct resource b3010_resources[] = {
    {
        .start  = 0x10000000,
        .end    = 0x10001000,
        .flags  = IORESOURCE_MEM,
...
static void b3010_dev_release( struct device * dev)
{
    return;
}

static struct platform_device b3010_dev = {
    .name           = DRIVER_NAME,
    .id             = -1,
    .num_resources  = ARRAY_SIZE(b3010_resources),
    .resource       = b3010_resources,
    .dev = {
        .release = b3010_dev_release,
    }
};
...
static int b3010_drv_probe(struct platform_device *pdev)
{
    struct resource *res;
    int ret = 0;
    int portid_count;
    ...
    fpga_data = devm_kzalloc(&pdev->dev, sizeof(struct b3010_fpga_data),
                             GFP_KERNEL);
    ...
    for (portid_count = 0 ; portid_count < VIRTUAL_I2C_PORT_LENGTH ; portid_count++) {
        ...
        fpga_data->i2c_adapter[portid_count] = b3010_i2c_init(pdev, portid_count, VIRTUAL_I2C_BUS_OFFSET);
    }
    ...
    for (portid_count = 0; portid_count < SFF_PORT_TOTAL; portid_count++) {
        ...
        if (i2c_adap) {
            fpga_data->sff_devices[portid_count] = b3010_sff_init(portid_count);
...
static int b3010_drv_remove(struct platform_device *pdev)
{
    int portid_count;
...
static struct platform_driver b3010_drv = {
    .probe  = b3010_drv_probe,
    .remove = __exit_p(b3010_drv_remove),
    .driver = {
        .name   = DRIVER_NAME,
    },
};
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    platform_device_register(&b3010_dev);
    platform_driver_register(&b3010_drv);
    return 0;
...
{
    platform_driver_unregister(&b3010_drv);
    platform_device_unregister(&b3010_dev);
    fpgafw_exit();
    pci_iounmap(pdev, fpga_dev.data_base_addr);
    pci_disable_device(pdev);
...
void b3010_exit(void)
{
    pci_unregister_driver(&pci_dev_ops);
}

module_init(b3010_init);
module_exit(b3010_exit);
...
MODULE_DESCRIPTION("Celestica b3010 switchboard platform driver");
