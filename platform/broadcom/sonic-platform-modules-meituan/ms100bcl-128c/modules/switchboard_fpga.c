/*
 * switchboard.c - Driver for mt3010 Switch FPGA/CPLD.
 *
...
#define CLASS_NAME "mt3010_fpga"
#define DRIVER_NAME "mt3010.switchboard"
#define FPGA_PCI_NAME "mt3010_fpga_pci"
#define DEVICE_NAME "fwupgrade"

static bool allow_unsafe_i2c_access=1;
...
/*
 * struct mt3010_fpga_data - Private data for mt3010 switchboard driver.
...
struct mt3010_fpga_data *fpga_data;
...
static struct device * mt3010_sff_init(int portid)
{
 ...
 *
 * mt3010 have 2 switch CPLDs. Each CPLD manages 28 ports.
 *
 *  +------------------+------------------+
...
reset_i2c_switch:
    // If retry was used up(retry = 0) and the last transfer result < 0
    if (retry <= 0 && error < 0)
    {
        retval = error;
        ...
        /**
         * mt3010: Device specific I2C reset topology
         */
...
static const struct i2c_algorithm mt3010_i2c_algorithm =
{
        ...
        int bus_number_offset)
{
    ...
    new_adapter->algo  = &mt3010_i2c_algorithm;
...
static struct i2c_adapter *mt3010_i2c_init(struct platform_device *pdev,
        int portid,
        int bus_number_offset)
{
...
// I/O resource need.
static struct resource mt3010_resources[] =
{
    {
        .start  = 0x10000000,
        .end    = 0x10001000,
...
static void mt3010_dev_release(struct device * dev)
{
    return;
}

static struct platform_device mt3010_dev =
{
    .name           = DRIVER_NAME,
    .id             = -1,
    .num_resources  = ARRAY_SIZE(mt3010_resources),
    .resource       = mt3010_resources,
    .dev = {
        .release = mt3010_dev_release,
    }
};
...
static int mt3010_drv_probe(struct platform_device *pdev)
{
    struct resource *res;
    int ret = 0;
    int portid_count;
    ...
    fpga_data = devm_kzalloc(&pdev->dev, sizeof(struct mt3010_fpga_data),
                             GFP_KERNEL);
    ...
    printk(KERN_INFO "allow_unsafe=0x%x \n", allow_unsafe_i2c_access);
    ...
    for (portid_count = 0 ; portid_count < VIRTUAL_I2C_PORT_LENGTH ; portid_count++)
    {
        ...
        fpga_data->i2c_adapter[portid_count] = mt3010_i2c_init(pdev, portid_count, VIRTUAL_I2C_BUS_OFFSET);
    }
    ...
    for (portid_count = 0; portid_count < SFF_PORT_TOTAL; portid_count++)
    {
        ...
        if (i2c_adap)
        {
            fpga_data->sff_devices[portid_count] = mt3010_sff_init(portid_count);
...
static int mt3010_drv_remove(struct platform_device *pdev)
{
    int portid_count;
    struct sff_device_data *rem_data;
    struct i2c_dev_data *adap_data;
    struct mt3010_fpga_data *priv;
...
static struct platform_driver mt3010_drv =
{
    .probe  = mt3010_drv_probe,
    .remove = __exit_p(mt3010_drv_remove),
    .driver = {
        .name   = DRIVER_NAME,
    },
};
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    platform_device_register(&mt3010_dev);
    platform_driver_register(&mt3010_drv);
    return 0;
...
{
    platform_driver_unregister(&mt3010_drv);
    platform_device_unregister(&mt3010_dev);
    fpgafw_exit();
    pci_iounmap(pdev, fpga_dev.data_base_addr);
    pci_disable_device(pdev);
...
void mt3010_exit(void)
{
    pci_unregister_driver(&pci_dev_ops);
}

module_init(mt3010_init);
module_exit(mt3010_exit);
...
MODULE_DESCRIPTION("Celestica mt3010 switchboard platform driver");
